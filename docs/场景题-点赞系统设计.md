...
> 代码仓库：[GitHub - l0sgAi/car-rental-backend at new_comment](https://github.com/l0sgAi/car-rental-backend/tree/new_comment)
...
>  完整代码请移步代码仓库：[GitHub - l0sgAi/car-rental-backend at new_comment](https://github.com/l0sgAi/car-rental-backend/tree/new_comment)

---

## II. 评论系统

评论区的评论为双层结构，使用`parentId`和`followCommentId`来表示回复和父子关系。顶级评论进行分页加载，回复则默认全部折叠，只显示回复数量，通过手动点击“查看`n`条回复”来手动分页获取。
